...
def draw_line(points):
    ...
    if point_1[0] == point_2[0]:
        ...
        for j in range(start, end + 1):
            map[j][point_1[0] - min_x + diff] = rock
    else:
        start = min(point_1[0],point_2[0]) - min_x + diff
        end = max(point_1[0],point_2[0]) - min_x + diff
...
src = '+'
map = [[air for i in range((3 * (max_y + 2)) + 1)] for j in range(max_y + 2)]
max_y += 2
map.append([rock for i in range((3 * max_y) + 1)])
diff = ((3 * max_y) -  (max_x - min_x)) // 2 
#diff += 1
src_r = 0
src_c = 500 - min_x + diff
draw_lines()
...
while is_running:
    ...
    while not has_stopped: 
        ...
        for move in moves:
            ...
            if poss_move[0] > max_y or poss_move[1] > ((3 * max_y)) or poss_move[1] < 0:
                is_running = False
                has_stopped = True
                has_moved = True
                break
